...
# Training function with L1 regularization and early stopping (NumPy version for MSE)
def train_numpy(model, loader_info, val_loader_info, loss_fn, loss_deriv_fn, optimizer, num_epochs, regularization=None, l1_lambda=0.0, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(loader_info['num_batches']):
            ...
            # Add L1 regularization to loss (if applicable)
            if regularization == 'l1' and l1_lambda > 0:
                l1_norm = 0
                for layer in model.layers:
                    # Check if layer has weights (e.g., LinearLayer, potentially ConvLayer later)
            ...
            running_loss += loss # Use the potentially regularized loss for tracking
            if (i + 1) % 100 == 0:
        ...
        if regularization == 'early_stopping' and val_loader_info:
            ...
            for i in range(val_loader_info['num_batches']):
                ...
                # Use the original loss function for validation loss calculation (without L1)
...
def test_numpy(model, loader_info):
    ...
    for i in range(loader_info['num_batches']):
        ...
        true_indices = np.argmax(labels_one_hot, axis=1) # Labels are one-hot
...
def objective_numpy(lr, num_epochs, batch_size, k_folds, nHidden_index, momentum, dropout_rate, l1_lambda):
    ...
    # momentum, dropout_rate, l1_lambda are passed directly

    # Note: weight_decay (L2) optimization is removed as it's not implemented in NumPy optimizer
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(np.arange(n_samples))):
        ...
        for reg in regularization_types:
            ...
            # Use SGD Optimizer (L2 weight_decay not supported)
            optimizer = SGD_Optimizer(model, learning_rate=lr, momentum=momentum)

            # Train using the modified train_numpy function
...
pbounds = {
    ...
    'l1_lambda': (0.00001, 0.001) # L1 lambda range (might need adjustment)
}

bo_optimizer = BayesianOptimization(f=objective_numpy, pbounds=pbounds, random_state=42, verbose=2)

# Run Bayesian optimization
print("Starting Bayesian Optimization (NumPy with L1/Dropout)...")
bo_optimizer.maximize(init_points=5, n_iter=10)

# Print best hyperparameters
print("\n=== Best Hyperparameters (NumPy with L1/Dropout) ===")
...
def train_final_model_numpy(params):
    ...
    # Use train as val for simplicity in final run
