...
def objective(lr, num_epochs, batch_size, k_folds, nHidden_index, momentum):
    num_epochs = int(num_epochs)
    batch_size = int(batch_size)
    k_folds = int(k_folds)
    nHidden_index = int(nHidden_index)
    momentum = momentum  # Momentum strength (beta_t)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_images)):
        ...
        optimizer = optim.SGD(model.parameters(), lr=lr, momentum=momentum)  # Add momentum
...
pbounds = {
    ...
    'momentum': (0.5, 0.99)  # Momentum range, including the suggested value 0.9
}
